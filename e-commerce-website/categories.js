...
const categories = [...new Set([...products.map(items => items.category)])]

// console.log(categories);
const options = categories.map(c => ({ value: c, label: c.split("-").join(' '), checked: false }))
...
// Now, new Set([...categories]) is used to create a Set from this array.A Set is a built -in JavaScript object that stores unique values.By spreading[...categories](using the spread operator), it takes the unique category values from the array and creates a Set, which automatically removes any duplicates.
//     So, the result is that categories will be a Set containing only unique category values from the products array.This is useful for obtaining a list of distinct categories present in the products.
...
//   "Java-Script";  initializes a string variable str with the value"Java-Script".

//  str.split("-") is used to split the string into an array of substrings based on the hyphen(`"-"`) delimiter.So, it will create an array like`["Java", "Script"]`.The `split` method splits the original string wherever it encounters the specified delimiter, and each part of the string is stored as an element in the resulting array.

// join('  ') is then used to join the elements of the array back together into a single string.In this case, it's using two spaces as the delimiter, so it will join the array elements with two spaces in between, resulting in the string `"Java  Script"`.

// So, the final result of this code is a string with two spaces between the words, transforming`"Java-Script"` into`"Java  Script"`.
